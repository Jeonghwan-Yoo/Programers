...
//     regex reWord(word + "[^a-z]", regex::icase);
//     regex reUrl("content=\"https://(.+?)\"\/>");
//     regex reLink("<a href=\"https://(.+?)\">");
...
//         l = pages[i].find("<body>");
//         r = pages[i].find("</body>");
//         for (auto j = sregex_iterator(pages[i].begin() + l, pages[i].begin() + r, reWord); j != sregex_iterator(); ++j)
//         {
//             if ((int)j->str().size() > (int)word.size() && isalpha(pages[i][j->position() + l - 1]))
//                 continue;
//             ++basic[i];
//         }
